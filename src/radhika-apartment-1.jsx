...
export default function App() {
  ...
  async function handleDownload(type, flatId = null) {
    setDlLoading(type);
    let path, name;
    if (type === "pdf") { path = "/reports/pdf/"; name = `maintenance_report_${new Date().toISOString().slice(0, 10)}.pdf`; }
    else if (type === "excel") { path = "/reports/excel/"; name = `maintenance_${new Date().toISOString().slice(0, 10)}.xlsx`; }
    else if (type === "receipt") { path = `/reports/receipt/${flatId}/`; name = `receipt_${flatId}.pdf`; }
